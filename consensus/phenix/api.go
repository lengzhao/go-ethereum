...
func (api *API) GetSnapshot(number *rpc.BlockNumber) (*Snapshot, error) {
	...
	return api.phenix.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
}
...
func (api *API) GetSigners(number *rpc.BlockNumber) ([]common.Address, error) {
	...
	snap, err := api.phenix.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
	if err != nil {
		return nil, err
	}
...
func (api *API) Proposals() map[common.Address]bool {
	api.phenix.lock.RLock()
	defer api.phenix.lock.RUnlock()

	proposals := make(map[common.Address]bool)
	for address, auth := range api.phenix.proposals {
...
func (api *API) Propose(address common.Address, auth bool) {
	api.phenix.lock.Lock()
	defer api.phenix.lock.Unlock()

	api.phenix.proposals[address] = auth
}
...
func (api *API) Discard(address common.Address) {
	...
	delete(api.phenix.proposals, address)
}
...
func (api *API) Status() (*status, error) {
	...
	for n := start; n < end; n++ {
		...
		sealer, err := api.phenix.Author(h)
		if err != nil {
			return nil, err
		}
		signStatus[sealer]++
