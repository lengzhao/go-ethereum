...
// Package phenix implements the proof-of-authority consensus engine.
package phenix
...
func New(config *params.PhenixConfig, db ethdb.Database) *Phenix {
...
func (c *Phenix) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
// from.
func (c *Phenix) verifySeal(snap *Snapshot, header *types.Header, parents []*types.Header) error {
...
// rewards given.
func (c *Phenix) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header) {
...
func (c *Phenix) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	// Sign all the things!
	sighash, err := signFn(accounts.Account{Address: signer}, accounts.MimetypePhenix, phenixRLP(header))
	if err != nil {
		return err
	}
...
func phenixRLP(header *types.Header) []byte {
	b := new(bytes.Buffer)
