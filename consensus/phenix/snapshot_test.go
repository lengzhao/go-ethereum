...
// checkpoint creates a Phenix checkpoint signer section from the provided list
...
// sign calculates a Phenix digital signature for the given block and embeds it
...
// the account may or may not have cast a Phenix vote.
...
// Tests that Phenix signer voting is evaluated correctly for various simple and
// complex scenarios, as well as that a few special corner cases fail correctly.
func TestPhenix(t *testing.T) {
	...
	for i, tt := range tests {
		...
		config.Phenix = &params.PhenixConfig{
			Period: 1,
			Epoch:  tt.epoch,
		}
		engine := New(config.Phenix, db)
		...
		// Pass all the headers through phenix and ensure tallying succeeds
